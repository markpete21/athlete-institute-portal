      ...
      {!session.userId ? (
        ...
        <>
          <div className="card-ink kpi">
            <span className="kpi-k">Balance</span>
            <span className="kpi-v">{surface.balance.toLocaleString()}</span>
            <span className="kpi-d">points ≈ {money(surface.balance)}</span>
          </div>
          ...
          <section className="flex flex-col gap-2">
            ...
            {surface.ledger.map((l, i) => (
              <div key={i} className="card flex items-center justify-between p-3 text-sm">
                ...
                <span className={`chip-delta ${l.delta > 0 ? 'up' : 'down'}`}>{l.delta > 0 ? '+' : ''}{l.delta}</span>
              </div>
            ))}
